...
#### 1. Description.md
- `Description.md`에는 참가자에게 보여줄 **문제 설명**을 작성합니다.
- 최소 포함 권장 항목:
  - 문제 배경/상황
  - 목표(무엇을 찾아야 하는지)
  - 제공 파일/접속 정보 안내
...
#### 2. Specfile
- `Specfile`에는 문제 메타데이터(문제명, 태그, 플래그 등)를 작성합니다.
...
tags  = misc                # 카테고리
flag  = 2026HL{...}         # 정답 플래그
internal_port = 5000/tcp    # 생략 가능 / 포트가 여러 개일 경우: 5000/tcp,1337/tcp
docker_compose = true       # 생략 가능 / DOCKER-COMPOSE를 사용한 경우 True
```
...
#### 4. 불필요한 파일 제외   
- .gitignore을 활용하여 아래와 같은 캐시/임시 파일은 저장소에 포함하지 마세요.   
